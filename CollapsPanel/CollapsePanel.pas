...
type
  ...
  private
    ...
    procedure SetStepCollaps(Value:Integer);
  ...
  published
    ...
    property StepCollaps:Integer read FStepCollaps write SetStepCollaps default 10;
...
begin
  if not IsCollapsed then
   begin
    ...
    repeat
        dec(i,FStepCollaps);
        if (i < HeaderPanel.Height+1) then begin
            i := HeaderPanel.Height+1;
            Collaps := True;
        end;
       Height := i;
    until(Collaps);

(*  for I:= FExpandedHeight downto (HeaderPanel.Height+1) do begin
...
var I:Integer;
    Collaps :Boolean;
begin
    if IsCollapsed then begin
        IsCollapsed:=False;

        //
        // Simple logic for Scrolling effect, with diff accelerations
        //
        Collaps := False;
        I:= FExpandedHeight;
        repeat
...
(*     for I:= (HeaderPanel.Height+1) to  FExpandedHeight do begin
            if FExpandedHeight < 300 then
                Height:=I
            else// 
                // if height larger than 300 then write to screen only when mod =0 is true, works like a step it in the for loop
                //
                if (I mod 4)=0 then    
                    Height:=I;
        end;
...
procedure TCollapsePanel.SetStepCollaps(Value: Integer);
begin
 FStepCollaps:=Value;
end;
